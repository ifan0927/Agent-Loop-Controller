...
printf '{"pr_number":1,"approver":"fixture-operator","source":"fixture_explicit_approval","approved_sha":"%s","ci_status":"pass","internal_review_sha":"%s","approved_at":"2026-07-11T00:00:00Z"}\n' "$candidate" "$candidate" >"$approval"

# Starting a second public CLI process is the required restart boundary.
go run "$repo_root/cmd/agentctl" local status "$run_id" --db "$lab/controller.db" \
  --requester fixture-operator --requester-database-id 1 --requester-node-id MDQ6VXNlcjE= --requester-type User
delivery_output="$(go run "$repo_root/cmd/agentctl" local fixture-deliver "$run_id" --db "$lab/controller.db" --registry "$lab/repository-registry.json" --approval "$approval")"
printf '%s\n' "$delivery_output"
printf '%s\n' "$delivery_output" | grep -q '"current_state": "completed"'

inspection_output="$(go run "$repo_root/cmd/agentctl" local inspect "$run_id" --db "$lab/controller.db" \
  --requester fixture-operator --requester-database-id 1 --requester-node-id MDQ6VXNlcjE= --requester-type User)"
printf '%s\n' "$inspection_output"
