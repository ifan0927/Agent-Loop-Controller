...
| Local binary upgrade | Independently cloned exact detached clean revision and build identity, unshared Git object storage, worker-owned single-link target, selected/opposite/legacy supervisor fencing, online SQLite backup under WAL activity, replacement/rollback/bootstrap/cleanup intent interruption, permanent post-intent rollback rejection, bounded heartbeat/process/config/schema/integrity observation, and exact-owned cleanup |
...
- local binary upgrade snapshot, replacement intent/effect/commit, rollback
  ...
  current-installation publication, and each exact-owned cleanup artifact.
...
schema version is 41. Opening a database applies missing forward migrations in a
