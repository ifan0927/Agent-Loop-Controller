...
| Disabled worker runtime | Indefinite process-lock and exact-digest heartbeat lifetime with idle no-candidate cycles, the closed legal onboarding outcome vocabulary with waiting/conflict parked and full heartbeat readback, zero normal-admission credential/request/reservation/run effects, serialized capacity-one onboarding continuation without fallback, quiescent integrity maintenance, typed enabled-to-disabled restart convergence, final-repository removal policy clearance, one-shot semantics, and clean cancellation |
| Configuration authority | Same-byte stable-root-flock-serialized filesystem binding/database-anchor/locator baseline crash matrix, trusted-ancestor and descendant-path replacement exclusion, private-leaf descriptor pinning across final directory sync plus post-sync owner/mode/link/path proof, initial private-directory parent-entry fsync and full-chain directory inode/mode proof, actual-VFS-fd exact-inode/private-mode proof including ABA replacement, idle reuse, pre/post transaction and row-consumption effects, and pool reconnect, post-reconcile operator reauthorization, accepted apply receipt reconciliation, historical same-digest no-op replay and immediate effective correlation for an already-loaded historical digest, external-drift fail-stop without offers or filesystem mutation, schema-v34 committed/accepted/ambiguous recovery evidence read/integrity compatibility, retired receipt rejection at application plus every SQLite producer/mutator boundary, unresolved historical recovery startup failure without settlement, active-run Linear task-source compatibility, no-replace single-link publication, pre-anchor temp/raw recovery, retryable raw/binding/locator/prune directory durability, bounded forward migration through v37, atomic no-op authority/receipt CAS, concurrent replay, intent-before-exchange crash matrix, captured-parent/exchange/cleanup fsync proof, durable prune claims plus serialized same-digest restaging, legacy non-elevation, manual-supervisor heartbeat, drift/effective convergence, development-fixture authority, and fail-closed direct plus automatic admission fencing |
| Repository lifecycle | Exactly-once positive and count-zero baseline adoption, deterministic empty digest and immutable ancestral configuration anchor, explicit zero-active-lifecycle replay, exact first-onboarding post-configuration bridge, complete initial unknown evidence, readiness precedence and stale-authority projection, read-only Git/GitHub/Linear/verifier observation, atomic publication, response-loss/restart ambiguity, enable/disable replay and CAS conflicts, authorization-before-lookup pagination, sanitized projection, and transaction-time manual plus automatic admission-token invalidation |
...
| Relocated cleanup fail-stop and historical compatibility | Schema-44-to-45 preservation of the v45 intent table, foreign key, immutable-authority and integrity triggers, registry membership, and operation-type constraint; current-schema startup plus historical `recover_cleanup_source` receipt detail/history/activity/integrity projection; explicit rejection at application and SQLite producer/mutator boundaries; no live CLI, service, Git repair, intent creation/resume/advance/settlement path, reset, force, rebind, or import; and unresolved ordinary cleanup evidence continuing to block repository removal |
| Existing-checkout onboarding | Read-only real-Git preflight with no object/ref/index mutation, unsafe/symlink/overlap rejection, exact local/remote base-head proof, sanitized path evidence, open/start/resume replay, monotonic durable retry attempts with one immutable activity per settlement, pre-start cancel, one-active-repository/source constraints, exact interrupted-resume migration and restart recovery, intent-before-effect step ordering, Linear lookup/create/reread, source-bound configuration exclusion, fresh worker convergence, disabled lifecycle creation, and complete readiness settlement |
| Routine queries | Stable configured-operator Controller-read authority for Runs and Controller Attention without historical authority scans or per-row predicates, canonical-repository plus Active/Ended/All filtering before count/order/page, reader/filter/target-bound keyset cursors stable across normal collection growth, target-specific authorization unchanged, deterministic sanitized digests, application-owned phase and wait assessment, fixed eleven-gate order, exact-head invalidation, conservative attention supersession, latest-complete queue reads, pre-binding onboarding discovery, schema-v2 heartbeat compatibility, and read-only settings convergence |
| Local binary upgrade | Independently cloned exact detached clean revision and build identity, unshared Git object storage, worker-owned single-link target, selected/opposite/legacy supervisor fencing, online SQLite backup under WAL activity, replacement/rollback/bootstrap/cleanup intent interruption, permanent post-intent rollback rejection, bounded heartbeat/process/config/schema/integrity observation, eligible post-bootstrap readiness-attention successor preparation including exact legacy convergence-exhaustion evidence and malformed/generic-pending rejection, immutable bidirectional predecessor/successor lineage, active-pointer replay, failed-candidate-only successor rollback, fresh full-backup confirmation, exact current-installation validation, active-unlink-before-root-sync cleanup commit with a complete bundle, every post-commit partial exact-owned reclamation position, legacy bundle-missing ambiguity fail-stop with byte and metadata stability, retired database-relocation command/writer fencing, strict schema-v1-v3 and recovery-evidence-v1-v2 reads, deterministic closed-root historical classification, unresolved historical zero-mutation admission, cleanup-committed prepare fencing with post-commit cleanup resume, and complete exact historical-transfer continuation through only its ordinary successor lifecycle with predecessor retention and no later managed-upgrade generation |
...
- configuration raw staging, filesystem baseline binding, database anchor,
  ...
  desired settlement, external-drift fail-stop, historical recovery evidence
  startup rejection without mutation, effective observation, and retention
  pruning;
...
- terminal cleanup source relocation fail-stop behavior, absence of any live
  recovery command/service/adapter/store mutator, continued repository-removal
  blocking, and schema-v45 historical intent plus receipt migration, startup,
  detail/history/activity, and integrity read compatibility;
- local binary upgrade snapshot, replacement intent/effect/commit, rollback
  ...
  response loss without duplicate successors, retired database-relocation
  command and journal-writer rejection, strict historical schema-v1-v3 and
  recovery-evidence-v1-v2 reads, unresolved phase/active-pointer/linkage
  zero-mutation fail-stop, stable predecessor ordering, valid-upgrade-ID
  non-directory rejection, cleanup-commit interruption then prepare rejection
  and cleanup resume, exact completed transfer through its ordinary successor
  lifecycle with later prepare rejection, exact pre-v43
  `integrity_convergence_exhausted` successor attention plus malformed evidence
  rejection, repeated exact schema-v43 attempt-eight ready publication
  supersession and its generic-pending/malformed negative matrix, current-
  installation publication, each exact-owned cleanup artifact, post-commit
  reclamation without lineage reconstruction, and immutable terminal historical
  recovery-predecessor retention.
...
its three `repository_onboarding` integrity triggers. Fresh and migrated
no-baseline fixtures prove count-zero adoption, restart/response-loss replay,
zero lifecycle/readiness rows, explicit unmatched-active rejection, ancestral
anchor corruption rejection, and old-binary forward fencing. The first-profile
fixture separately proves both legal bridge statuses, exact generation/apply
receipt/step evidence, rejection of malformed or later-step candidates, worker
reopen, and eventual disabled lifecycle/readiness creation.
