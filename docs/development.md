...
| Relocated cleanup recovery | Schema-44-to-45 evidence preservation and migration-safe integrity-registry tracking, authorization-first configured-operator plus frozen-run scope, sanitized read-only preview, exact abandoned terminal attention/action/process/lease/slot/permit/scheduling/ownership CAS, private replacement-path replay, every-stage pre/post/final replacement-identity observation, exact absent local-ref proof, pre-effect/repaired/detached/removed topology adoption, argv-only worktree repair and durable owned-worktree HEAD detach with zero branch-ref mutation, full drift/dirty/ambiguity rejection, common receipt settlement, disposable real-Git/SQLite replay, and ordinary repository-removal guard clearance |
...
- terminal cleanup source relocation acceptance, operation-state rejection,
  ...
  worktree removal, normal cleanup settlement, common receipt observation, and
  response loss before and after every local effect;
