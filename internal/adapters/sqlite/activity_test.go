...
func TestActivityV43MigrationPreservesRowsAndTracksOnlySemanticRuntimeChanges(t *testing.T) {
	...
	if err := store.db.QueryRow(`SELECT COUNT(*) FROM sqlite_master WHERE type='trigger' AND name LIKE 'integrity_track_%'`).Scan(&migratedTriggers); err != nil || migratedTriggers != legacyTriggers+6 {
