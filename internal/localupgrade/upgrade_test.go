...
func writeRetainedCleanupPredecessor(t *testing.T, manager *Manager, successor journal, idCharacter string, recovery bool) journal {
	...
	if recovery {
		writeHistoricalJournalFixture(t, manager, bundle, predecessor)
	} else if err := writeJournal(bundle, predecessor, manager.uid); err != nil {
		t.Fatal(err)
	}
	return predecessor
}

func writeHistoricalJournalFixture(t *testing.T, manager *Manager, bundle string, value journal) {
	t.Helper()
	if err := validateJournal(value, value.UpgradeID); err != nil {
		t.Fatalf("historical journal fixture is invalid: %v", err)
	}
...
func TestLegacyBundleMissingUnsafeEvidenceFailsWithoutMutation(t *testing.T) {
	tests := []struct {
		...
		{name: "recovery-bearing retained journal", mutate: func(t *testing.T, manager *Manager, successor journal) {
			...
			writeHistoricalJournalFixture(t, manager, manager.bundlePath(predecessor.UpgradeID), predecessor)
		}},
